...
import { parsePhpSessionData } from './logic';

export default function PhpSessionDataTool({ locale, toolMeta }: { locale: string; toolMeta: ToolMeta }) {
  const t = useTranslations('tools.php-session-data');
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [error, setError] = useState('');
  const handleParse = () => { const r = parsePhpSessionData(input); if (r.error) { setError(r.error); setOutput(''); } else { setOutput(r.output!); setError(''); } };
  return (
