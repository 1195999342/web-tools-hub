...
import { stripHtml } from './logic';

export default function HtmlStripTool({ locale, toolMeta }: { locale: string; toolMeta: ToolMeta }) {
  const t = useTranslations('tools.html-strip');
  ...
  return (
    <ToolLayout toolMeta={toolMeta} locale={locale as any} instructions={t('instructions')}>
      <div className="flex flex-col gap-4">
        ...
        <Button onClick={() => setOutput(stripHtml(input))}>{t('strip_button')}</Button>
        {output && (
