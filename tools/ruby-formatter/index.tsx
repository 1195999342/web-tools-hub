...
import { formatRubyCode } from './logic';

export default function RubyFormatterTool({ locale, toolMeta }: { locale: string; toolMeta: ToolMeta }) {
  const t = useTranslations('tools.ruby-formatter');
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const handleFormat = () => setOutput(formatRubyCode(input));
  return (
