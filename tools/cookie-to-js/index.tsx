...
import { cookieToJs } from './logic';

export default function CookieToJsTool({ locale, toolMeta }: { locale: string; toolMeta: ToolMeta }) {
  const t = useTranslations('tools.cookie-to-js');
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const handleConvert = () => setOutput(cookieToJs(input));
  return (
