...
export default function MorseCodeTool({ locale, toolMeta }: { locale: string; toolMeta: ToolMeta }) {
  const t = useTranslations('tools.morse-code');
  const [tab, setTab] = useState('encode');
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');

  const handleConvert = () => setOutput(tab === 'encode' ? textToMorse(input) : morseToText(input));

  return (
    <ToolLayout toolMeta={toolMeta} locale={locale as any} instructions={t('instructions')}>
      <div className="flex flex-col gap-4">
        <Tabs tabs={[{ id: 'encode', label: t('text_to_morse') }, { id: 'decode', label: t('morse_to_text') }]} activeTab={tab} onTabChange={id => { setTab(id); setInput(''); setOutput(''); }} />
