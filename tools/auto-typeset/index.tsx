...
import { autoTypeset } from './logic';

export default function AutoTypesetTool({ locale, toolMeta }: { locale: string; toolMeta: ToolMeta }) {
  const t = useTranslations('tools.auto-typeset');
  ...
  return (
    <ToolLayout toolMeta={toolMeta} locale={locale as any} instructions={t('instructions')}>
      <div className="flex flex-col gap-4">
        ...
        <Button onClick={() => setOutput(autoTypeset(input))}>{t('format_button')}</Button>
        {output && (
