...
import { formatCCode } from './logic';

export default function CFormatterTool({ locale, toolMeta }: { locale: string; toolMeta: ToolMeta }) {
  const t = useTranslations('tools.c-formatter');
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const handleFormat = () => setOutput(formatCCode(input));
  return (
