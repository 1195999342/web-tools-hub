...
export default function HtmlEscapeTool({ locale, toolMeta }: { locale: string; toolMeta: ToolMeta }) {
  ...
  const tabs = [
    { id: 'escape', label: t('escape_tab') },
    { id: 'unescape', label: t('unescape_tab') },
  ];

  function handleAction() {
    setError('');
    const result = activeTab === 'escape' ? escapeHtml(input) : unescapeHtml(input);
  ...
  return (
    <ToolLayout toolMeta={toolMeta} locale={locale as any} instructions={t('instructions')}>
      <div className="flex flex-col gap-4">
        ...
        <Button onClick={handleAction}>{activeTab === 'escape' ? t('escape_button') : t('unescape_button')}</Button>
