...
import { parsePhpSession } from './logic';

export default function PhpSessionTool({ locale, toolMeta }: { locale: string; toolMeta: ToolMeta }) {
  const t = useTranslations('tools.php-session');
  const [input, setInput] = useState('');
  const [output, setOutput] = useState('');
  const [error, setError] = useState('');
  const handleParse = () => { const r = parsePhpSession(input); if (r.error) { setError(r.error); setOutput(''); } else { setOutput(r.output!); setError(''); } };
  return (
